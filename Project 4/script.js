...
function exportAsMP3() {
    ...
    setTimeout(() => {
        try {
            ...
            // 将 Blob 转换为 Base64 用于内嵌到 HTML
            const reader = new FileReader();
            reader.onloadend = () => {
                // 移除 data:audio/mp3;base64, 前缀
                lastRecordedBase64 = reader.result.split(',')[1];
...
<head>
    ...
    <title>🎵 Button Sounds Player</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="player-container">
        <div class="logo">🎹</div>
        <h1>Button Sounds</h1>
        <p class="subtitle">Created with BUTTONS ARCHIVE</p>
        ...
        <div class="audio-wrapper">
            <audio controls autoplay>
                <source src="data:audio/mp3;base64,${lastRecordedBase64}" type="audio/mp3">
                您的浏览器不支持音频播放
            </audio>
            <p class="play-hint">点击播放按钮收听录音</p>
        </div>
        
        <p class="timestamp">录制时间: ${timestamp}</p>
        
        <div class="footer">
            <p class="footer-text">Made with ❤️ by BUTTONS ARCHIVE</p>
        </div>
    </div>
</body>
</html>`;
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `button-sounds-player-${Date.now()}.html`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
...
function generateQRCode() {
    ...
    if (!url) {
        alert('请输入播放器页面的URL地址！');
        return;
    }
    
    // 简单的URL验证
    if (!url.startsWith('http://') && !url.startsWith('https://')) {
        alert('请输入有效的URL地址（以 http:// 或 https:// 开头）');
        return;
    }
